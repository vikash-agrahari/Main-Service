...
export class TwilioCommService {
  public constructor(private readonly twilioService: TwilioService, private readonly configService: ConfigService) {}

  async sendOtp(message: string, receiver: string): Promise<any> {
    return new Promise((resolve) => {
      try {
        if (this.configService.get('ENV') !== ENUM.ENVIRONMENT.PRODUCTION && this.configService.get('ENV') !== ENUM.ENVIRONMENT.PRE_PROD) {
          resolve({
            code: 'queued',
          });
        } else {
          //const senderNumber = this.configService.get('TWILIO_PHONE_NO');
          const messagingServiceSid = this.configService.get<string>('TWILIO_MESSAGING_SID');
          const smsPayload = {
            body: message,
            //from: senderNumber,
